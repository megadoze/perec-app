...
export default function ClientHome({
  ...
  return (
    <>
      ...
      <LazyCategoryFourH news={politics} withText locale={locale} />

      <div className="my-6 border-t border-neutral-100 dark:border-gray-800" />
      <h2 className="font-narrow text-2xl pb-6 text-red-600 dark:text-red-500">
        <Link href={`/${locale}/economics`}>{t.economics}</Link>
      </h2>
      <LazyCategoryFourH news={economics} withPhoto withText locale={locale} />

      <div className="my-6 border-t border-neutral-100 dark:border-gray-800" />
      <h2 className="font-narrow text-2xl pb-6 text-red-600 dark:text-red-500">
        <Link href={`/${locale}/life`}>{t.life}</Link>
      </h2>
      <LazyCategorySix news={life} withPhoto withText locale={locale} />

      <div className="my-6 border-t border-neutral-100 dark:border-gray-800" />
      <h2 className="font-narrow text-2xl pb-6 text-red-600 dark:text-red-500">
        <Link href={`/${locale}/culture`}>{t.culture}</Link>
      </h2>
      <LazyCategorySixV news={culture} locale={locale} />

      <div className="my-6 border-t border-neutral-100 dark:border-gray-800" />
      <h2 className="font-narrow text-2xl pb-6 text-red-600 dark:text-red-500">
        <Link href={`/${locale}/media`}>{t.media}</Link>
      </h2>
      <LazyCategoryMediaFour news={media} withPhoto locale={locale} />
    </>
  );
}
